/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { JobVacancyWhereUniqueInput } from "./JobVacancyWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { JobVacancyUpdateInput } from "./JobVacancyUpdateInput";

@ArgsType()
class UpdateJobVacancyArgs {
  @ApiProperty({
    required: true,
    type: () => JobVacancyWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => JobVacancyWhereUniqueInput)
  @Field(() => JobVacancyWhereUniqueInput, { nullable: false })
  where!: JobVacancyWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => JobVacancyUpdateInput,
  })
  @ValidateNested()
  @Type(() => JobVacancyUpdateInput)
  @Field(() => JobVacancyUpdateInput, { nullable: false })
  data!: JobVacancyUpdateInput;
}

export { UpdateJobVacancyArgs as UpdateJobVacancyArgs };
